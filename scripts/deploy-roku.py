...
def deploy_to_roku():
    roku_ip = sys.argv[1] if len(sys.argv) > 1 else os.environ.get("ROKU_IP", "10.0.0.171")
    roku_pass = sys.argv[2] if len(sys.argv) > 2 else os.environ.get("ROKU_DEV_PASSWORD", "sobeoapp")
    ...
    if not os.path.exists(zip_file):
        print("📦 roku-channel.zip não encontrado. Gerando pacote primeiro...")
    ...
    try:
        ...
        body.append(f"--{boundary}\r\nContent-Disposition: form-data; name=\"archive\"; filename=\"{zip_file}\"\r\nContent-Type: application/zip\r\n\r\n".encode("utf-8"))
